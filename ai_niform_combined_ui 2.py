...
class GuardUI(QWidget):
    ...
    def show_guard_selection(self):
        ...
        img_label = QLabel()
        img_path = '/Users/ichiroyamazaki/Documents/AIniform/ux-users/image/guard-screen.png'
        pixmap = QPixmap(img_path)
        if not pixmap.isNull():
            scaled = pixmap.scaled(600, 350, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            img_label.setPixmap(scaled)
            img_label.setAlignment(Qt.AlignCenter)
        else:
            img_label.setText('[Image not found]')
            img_label.setStyleSheet('color: #888; font-size: 18px; font-style: italic;')
            img_label.setAlignment(Qt.AlignCenter)
        center_vlayout.addWidget(img_label)
    ...
    def show_manual_verification(self, user_info=None, check_in_time=None):
        ...
        # Add a stretchable space at the top to push content down
    ...
    def show_approved_confirmation_screen(self, user_info=None, check_in_time=None):
        self.clear_central_layout()
        main_widget = QWidget()
        main_layout = QHBoxLayout(main_widget)
        main_layout.setContentsMargins(0, 0, 0, 0)
        ...
        right_layout.setContentsMargins(30, 30, 30, 30)
        right_layout.setSpacing(18)
        # Use the provided user_info or the manual_user_info if available
        display_user_info = user_info if user_info is not None else self.manual_user_info
        display_check_in_time = check_in_time if check_in_time is not None else self.manual_check_in_time
        ...
        if display_user_info:
            ...
            right_layout.addWidget(photo_label, alignment=Qt.AlignCenter)
            # Name
            name_label = QLabel(display_user_info.get('name', ''))
            name_label.setFont(QFont('Arial', 28, QFont.Bold))
            name_label.setStyleSheet('color: #fff;')
            name_label.setAlignment(Qt.AlignCenter)
            right_layout.addWidget(name_label)
            # Role
            role_label = QLabel(f"({display_user_info.get('role', '')})")
            role_label.setFont(QFont('Arial', 20, QFont.Bold))
            role_label.setStyleSheet('color: #b8e0ff;')
            role_label.setAlignment(Qt.AlignCenter)
            right_layout.addWidget(role_label)
            ...
            right_layout.addWidget(checkin_label)
            # Check-in time
            time_label = QLabel(display_check_in_time if display_check_in_time else '')
            time_label.setFont(QFont('Arial', 20, QFont.Bold))
            time_label.setStyleSheet('color: #fff;')
            time_label.setAlignment(Qt.AlignCenter)
            right_layout.addWidget(time_label)
    ...
    def show_logo_user_info_screen(self, user_info=None, check_in_time=None):
        ...
        left_layout.setSpacing(0)
        logo_label = QLabel()
        # Change logo path to guard-screen.png
        logo_path = '/Users/ichiroyamazaki/Documents/AIniform/ux-users/image/guard-screen.png'
        logo_pixmap = QPixmap(logo_path)
        if not logo_pixmap.isNull():
            scaled = logo_pixmap.scaled(600, 350, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            logo_label.setPixmap(scaled)
        else:
            logo_label.setText('[Logo]')
        logo_label.setAlignment(Qt.AlignCenter)
    ...
    def show_special_pass_close_screen(self, user_info=None, check_in_time=None, check_out_time=None):
        self.clear_central_layout()
        main_widget = QWidget()
        main_layout = QHBoxLayout(main_widget)
        main_layout.setContentsMargins(0, 0, 0, 0)
        ...
        if user_info:
            ...
            # Check-in/out label and time
            mode = user_info.get('mode', '').lower() if user_info else ''
            if mode == 'check out':
                check_label = QLabel('Time Check Out:')
                check_label.setFont(QFont('Arial', 16, QFont.Bold))
                check_label.setStyleSheet('color: #fff; margin-top: 20px;')
                check_label.setAlignment(Qt.AlignCenter)
                right_layout.addWidget(check_label)
                time_label = QLabel(check_out_time if check_out_time else '')
            else:
                check_label = QLabel('Time Check In:')
                check_label.setFont(QFont('Arial', 16, QFont.Bold))
                check_label.setStyleSheet('color: #fff; margin-top: 20px;')
                check_label.setAlignment(Qt.AlignCenter)
                right_layout.addWidget(check_label)
...
class STIMonitor(QWidget):
    def __init__(self, guard_window=None):
        ...
        # Set fixed resolution to 1920x1080
        ...
        self.group_photo_label.installEventFilter(self)
    ...
    def initUI(self):
        ...
        # Set the initial pixmap, scaling will be handled by the event filter
        if not self.group_photo_pixmap.isNull():
            self.group_photo_label.setPixmap(self.group_photo_pixmap)
        else:
    ...
    def run_camera_detection(self):
        ...
        def detection_thread():
            try:
                process = subprocess.Popen(['python3', 'test ict.py', '0'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1, universal_newlines=True)
    ...
    def show_verified_screen(self):
        ...
        # After successful camera verification, also trigger the GuardUI's approved confirmation flow
        if hasattr(self, 'guard_window') and self.guard_window and not self.guard_window.manual_verification_active:
            self.guard_window.show_approved_confirmation_screen(self.user_info, self.check_in_time)
    ...
    def show_manual_verification_result(self, approved):
        if approved:
            # Call the guard window's approved confirmation screen
            if hasattr(self, 'guard_window') and self.guard_window:
                self.guard_window.show_approved_confirmation_screen(self.user_info, self.check_in_time) # Pass user_info and check_in_time
            # Show the verified screen on the main monitor
            self.show_verified_screen() # Call show_verified_screen instead of reset_instruction
        else:
            # Call the guard window's denied wait screen
            if hasattr(self, 'guard_window') and self.guard_window:
                self.guard_window.show_denied_wait_screen()

                # Show a waiting message on the main screen and then reset
                for i in reversed(range(self.right_layout.count())):
                    widget = self.right_layout.itemAt(i).widget()
                    if widget:
                        widget.setParent(None)
                ...
                self.header_label.setStyleSheet("background-color: #F4A940; color: #222; padding: 36px 0; min-height: 70px;") # Match orange background and main screen header style

                # Show welcome1.jpeg in the left panel
                img_path = '/Users/ichiroyamazaki/Documents/AIniform/ux-users/image/welcome1.jpeg'
                self.group_photo_pixmap = QPixmap(img_path)
                if not self.group_photo_pixmap.isNull():
                    label_size = self.group_photo_label.size()
                ...
                # Show user info and denial message on the right
                if self.user_info:
                    name_label = QLabel(self.user_info.get('name', ''))
                    name_label.setFont(QFont('Arial', 28, QFont.Bold))
                    name_label.setStyleSheet('color: #fff;')
                    ...
                    report_label = QLabel('Report has been generated.')
                    report_label.setFont(QFont('Arial', 20, QFont.Bold))
                    report_label.setStyleSheet('color: #fff;')
                    report_label.setAlignment(Qt.AlignCenter)
                    self.right_layout.addWidget(report_label)

                    # Only show parent/guardian subtext for students
                    if self.user_info.get('role', '').lower() == 'student':
                        parent_label = QLabel('Parents/Guardian has been\nalso notified.')
                        parent_label.setFont(QFont('Arial', 16))
                    ...
                    self.right_layout.addWidget(spacer2)
                ...
                # Reset the STIMonitor UI after a delay
                QTimer.singleShot(3000, self.reset_instruction) # Adjust delay as needed
